...
void receiveIR_wererabbit() {
  if (irrecv.decode(&results)) {
      ...
      if(ir_value/10000 == 44){
        ...
        controllo_colpito_wererabbit(id_last_wolf);
      }
...
void receiveIR_infected() {
  if (irrecv.decode(&results)) {
      ...
      if(ir_value/10000 == 44){
        id_wolf = (ir_value%440000)/100;
        int id_last_wolf = (ir_value%(440000 + id_wolf*100));
        controllo_colpito_infected(id_last_wolf);
      }
      Serial.println(id_wolf);
      irrecv.resume(); // Receive the next value
  }
...
void controllo_colpito_wererabbit(int id_last_wolf){
  if(id_last_wolf == 0){
...
void controllo_colpito_infected(int id_last_wolf){
  if(id_last_wolf == 0){
    if(id_wolf== myPlayerID){
      stato = 0;
    }
